...
function __init__()
{
	...
	clientfield::register( "zbarrier",	 	"pap_light", 		VERSION_DLC1, 2, "int" );
}
...
function private spawn_init()
{
	...
	for ( i = 0; i < zbarriers.size; i++ )
	{
		...
		radius = level.pack_a_punch.interaction_trigger_radius;
		
		if ( isDefined( zbarriers[ i ].script_string ) && zbarriers[ i ].script_string == "ritual_pap" )
			radius = 140;
		else if ( isDefined( zbarriers[ i ].script_string ) && ( zbarriers[ i ].script_string == "t7_castle" || zbarriers[ i ].script_string == "t7_origins" ) )
			radius = 100;
		
		use_trigger = Spawn( "trigger_radius_use", zbarriers[i].origin + (0, 0, level.pack_a_punch.interaction_height), 0, radius, level.pack_a_punch.interaction_trigger_height );
		...
		// Set up sounds
		if ( isDefined( zbarriers[i].script_string ) && ( zbarriers[i].script_string == "t7_castle" || zbarriers[i].script_string == "t7_origins" ) )
		{
...
function private third_person_weapon_upgrade( current_weapon, upgrade_weapon, packa_rollers, pap_machine, trigger )
{
	...
	if ( isDefined( trigger.zbarrier.script_string ) && ( trigger.zbarrier.script_string == "t7_castle" || trigger.zbarrier.script_string == "t7_origins" ) )
		self playsound( "zmb_perks_castle_packa_upgrade" );
	else
	...
	{
		if ( isDefined( trigger.zbarrier.script_string ) && ( trigger.zbarrier.script_string == "t7_castle" || trigger.zbarrier.script_string == "t7_origins" ) )
			self playsound( "zmb_perks_castle_packa_ready" );
		else
...
function private vending_weapon_upgrade()
{
	...
	if ( power_off )
	{
		...
		level waittill("Pack_A_Punch_on");
		//iPrintLnBold( "PACK TURNED ON" );
		
		if ( isDefined( self.zbarrier.script_string ) && ( self.zbarrier.script_string == "t7_castle" || self.zbarrier.script_string == "t7_origins" ) )
			self.zbarrier clientfield::set("pap_light", 2);
		else 
			self.zbarrier clientfield::set("pap_light", 1);
	...
	{
		origins = 1;
		pap_machine playloopsound("zmb_perks_castle_packa_loop");
	}
	else
	...
	self thread shutOffPAPSounds( pap_machine, packa_rollers, packa_timer, origins );
...
function private shutOffPAPSounds( ent1, ent2, ent3, origins = false )
{
	while(1)
	{
		level waittill( "Pack_A_Punch_off" );
		level thread turnOnPAPSounds( ent1, origins );
...
function private turnOnPAPSounds( ent, origins )
{
	level waittill( "Pack_A_Punch_on" );
	if ( IS_TRUE( origins ) )
		ent playloopsound( "zmb_perks_castle_packa_loop" );
	else
...
function private wait_for_player_to_take( player, weapon, packa_timer, b_weapon_supports_aat, isRepack )
{
	...
	{
		if ( isDefined( self.zbarrier.script_string ) && ( self.zbarrier.script_string == "t7_castle" || self.zbarrier.script_string == "t7_origins" ) )
			packa_timer playloopsound( "zmb_perks_castle_packa_loop" );
		else
